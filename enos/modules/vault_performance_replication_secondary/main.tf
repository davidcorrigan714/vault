...
variable "secondary_token" {
  type = map(object({
    wrapping_token               = string
    wrapping_accessor            = string
    wrapping_token_ttl           = string
    wrapping_token_creation_time = string
    wrapping_token_creation_path = string
  }))
  description = "The wrapping token created on primary cluster"
}

locals {
  wrapping_token = lookup(var.secondary_token, "wrapping_token", null)
}
...
resource "enos_remote_exec" "configure_pr_secondary" {
  content = templatefile("${path.module}/templates/configure-vault-pr-secondary.sh", {
    ...
    wrapping_token          = local.wrapping_token
  })
